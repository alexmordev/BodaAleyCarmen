...
export function getPool() {
  if (!pool) {
    pool = mysql.createPool({
      ...
      // La MySQL de Hostinger limita a max_user_connections (~75). Mantenemos un
      // pool pequeno y soltamos conexiones ociosas rapido para no acaparar cupo
      // (sobre todo si local y produccion comparten el mismo usuario de BD).
      connectionLimit: 5,
      maxIdle: 2,
      idleTimeout: 30000,
      queueLimit: 0,
